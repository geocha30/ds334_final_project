...
In general, what we have seen from the above plots is that attempting to fit a Logistic Regression Model give different combinations of proxies does not do a good job. Therefore, a more complex model might be able to interpret the relationhips between the various variables that we have. 

```{r}
library(keras)

# Preprocess the data
scaled_data <- scale(full_stats_df[, sapply(full_stats_df, is.numeric)])
set.seed(123)
